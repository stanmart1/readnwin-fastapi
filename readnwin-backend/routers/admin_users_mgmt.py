...
def get_admin_without_active_check(
...
def get_all_users(
    ...
    return_all: bool = Query(
        False, description="Return all matching results without pagination"
    ),
    ...
    try:
        ...
        # Return all results if return_all is True, otherwise paginate
        if return_all:
            users = query.all()
        else:
        ...
        return {
            ...
            "total": total,
            "skip": skip if not return_all else 0,
            "limit": limit if not return_all else total,
            "has_more": (skip + limit) < total if not return_all else False,
        }
